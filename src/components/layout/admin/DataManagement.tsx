...
function DataManagement({ products, orders, users }: { products: Array<AdminPanelProduct>, orders: Array<AdminPanelOrder>, users: Array<AdminPanelUser> }) {
        ...
        (state, productEntity: AdminPanelProduct) => {
            ...
            if (exists) {
                return state.map(product =>
                    product.productId === productEntity.productId ? productEntity : product
                );
            }

            return [...state, productEntity];
        }
    );
    ...
    const handleSave = async (savedProduct: AdminPanelProduct) => {
        ...
        try {
            if (sheetMode === 'add') {
                const result = await createProduct({ data: optimisticProduct })
                if (result) {
                    setProductsState([...productsState, result]);
                }
            } else {
                const result = await updateProduct({ data: savedProduct })
                if (result) {
                    setProductsState(productsState.map(product => product.productId === result.productId ? result : product));
                }
            }
            toast.success(`Product ${sheetMode === 'add' ? "added" : "updated"} successfully!`);
        } catch (error) {
            console.error(error)
            toast.error(`Failed to ${sheetMode === 'add' ? "add" : "update"} product. Please try again.`);
        }
    };

    const handleDelete = (tab: keyof typeof tableColumns) => {
        if (tab === 'products') {
            setProductsState(productsState.filter(product => !selectedItems.products.has(product.productId)));
        ...
        <>
            <Tabs id='Data Managing Dashboard' defaultValue={activeTab}>
                ...
                <TabsContent value='products'>
                    <ProductsTable
                        products={optimisticProducts?.length ? optimisticProducts : []}
            ...
            <thead className='z-25 sticky top-28 bg-gray-50'>
                <tr>
                    <th className='w-10 px-6 py-3 text-left text-xs font-medium uppercase text-gray-600 tracking-wide'>
                        <Checkbox className='cursor-pointer' checked={selectedItems.size === orders.length} onCheckedChange={() => toggleSelectAll(allIds)} />
                    </th>
                        ...
                        return (
                            <>
                                <tr className='hover:bg-yellow-50'>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{order.orderId.slice(0, 5)}...</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{order.shippingState}</td>
                                </tr>
                                            ...
                                            <div className="grid grid-cols-1 gap-5">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-gray-700 text-sm">{order.shippingAddress1}, {order.shippingAddress2}, {order.shippingCity}, {order.shippingState} {order.shippingPostalCode}</p>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="space-y-1 text-xs">
                                                            <div className="text-gray-700 flex gap-2"><span className="">Placed</span><span>{format.dateD(order.createdAt)} at {format.timeD(order.createdAt)}</span></div>
                                            ...
                                            <div className="grid grid-cols-1 gap-5">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-semibold uppercase tracking-wider mb-2">Metadata</p>
                                                        <div className="space-y-1 text-xs">
                                                            <div className="text-gray-700 flex gap-2"><span className="">Updated: </span><span>{format.dateD(product.updatedAt)} at {format.timeD(product.updatedAt)}</span></div>
            ...
            <tbody className='divide-y divide-gray-200'>
                {users.map(user => {
                    ...
                    return (
                        <tr className='hover:bg-yellow-50'>
                            <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-900 flex items-center gap-2'>
                                <Checkbox checked={selectedItems.has(user.id)} onCheckedChange={() => toggleSelect(user.id)} />
                                <button
                                    type="button"
                            ...
                            <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-900'>{user.id.slice(0, 5)}...</td>
