...
function DataManagement() {
    ...
    const handleSave = (savedProduct: Product) => {
        if (sheetMode === 'add') {
            setProductsState([...productsState, savedProduct]);
        } else {
            setProductsState(productsState.map(p => p.productId === savedProduct.productId ? savedProduct : p));
        }
        setSheetOpen(false);
    };
    ...
    const handleDelete = (tab: keyof typeof tableColumns) => {
        if (tab === 'products') {
            setProductsState(productsState.filter(p => !selectedItems.products.has(p.productId)));
