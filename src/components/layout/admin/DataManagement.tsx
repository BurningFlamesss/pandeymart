...
function OrdersTable() {
    ...
    const toggleAll = () =>
        setSelectedRows(
            selectedRows.size === INITIAL_ORDERS.length
                ? new Set()
                : new Set(INITIAL_ORDERS.map((order) => order.orderId))
        );
            ...
            <thead className='bg-gray-50'>
                <tr>
                    <th className='w-10 px-6 py-3 text-left text-xs font-medium uppercase text-gray-600 tracking-wide'>
                        <Checkbox className='cursor-pointer' checked={selectedRows.size === INITIAL_ORDERS.length} onCheckedChange={toggleAll} />
                    </th>
