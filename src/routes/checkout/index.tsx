...
function removeSavedAddress(addressId: string): Array<SavedAddress> {
    const updated = getSavedAddresses().filter((address) => address.id !== addressId);
    persistSavedAddresses(updated);
    return updated;
}
...
function calculateItemUnitPrice(
    ...
    return basePrice + customizations.reduce(
        (groupSum, group) =>
...
function validateCartAgainstProducts(
    ...
    const validations: Array<ValidationResult> = cart.map((cartItem) => {
        ...
        if (cartItem.customizations?.length) {
            ...
            cartItem.customizations.forEach((cartGroup) => {
                const productGroup = product.customizations?.find(
                    (group) => group.title === cartGroup.title
                );
                if (productGroup) {
                    cartGroup.options.forEach((cartOption) => {
                        const productOption = productGroup.options.find(
                            (option) => option.label === cartOption.label
                        );
    ...
    return {
        validations,
        hasIssues: validations.some(
            (validation) => validation.issues.length > 0 || !validation.isAvailable
        ),
    };
}

function ValidationBanner({ validations }: { validations: Array<ValidationResult> }) {
    const itemsWithIssues = validations.filter((validation) => validation.issues.length > 0);
    if (itemsWithIssues.length === 0) return null;

    return (
        <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <p className="text-xs font-semibold uppercase text-amber-700 mb-2">Update the Cart</p>
...
function RouteComponent() {
    ...
    const { data: realProducts = [], isPending, isError } = useQuery({
        queryKey: ["checkout-products", cart.map((cartItem) => cartItem.productId).sort().join(",")],
        queryFn: () => {
            if (cart.length === 0) return [];
            const uniqueProductIds = [...new Set(cart.map((cartItem) => cartItem.productId))];
    ...
    const { subtotal, shippingCost, grandTotal } = useMemo(() => {
        const subtotalAmount = cart.reduce((sum, cartItem) => {
            const validation = validations.find((validation) => validation.cartItemId === cartItem.cartItemId);
    ...
    const unavailableItems = validations.filter((validation) => !validation.isAvailable);
    ...
    function maybeSaveAddress() {
        if (!saveForLater || selectedSavedAddressId !== null) return;
        const newAddress: SavedAddress = {
            id: crypto.randomUUID(),
            label: `${form.city}, ${form.addressLine1}`,
        ...
        const updated = addSavedAddress(newAddress);
        setSavedAddresses(updated);
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 lg:gap-16">
                            ...
                            <>
                                    ...
                                    <div className="space-y-10">
                                        <section>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <FormField label="First Name" name="firstName" span="half" placeholder="Jane" value={form.firstName} error={errors.firstName} onChange={(value) => updateField("firstName", value)} />
                                                <FormField label="Last Name" name="lastName" span="half" placeholder="Doe" value={form.lastName} error={errors.lastName} onChange={(value) => updateField("lastName", value)} />
                                                <FormField label="Email" name="email" type="email" placeholder="jane@example.com" value={form.email} error={errors.email} onChange={(value) => updateField("email", value)} />
                                                <FormField label="Phone" name="phone" type="tel" placeholder="+977 98XXXXXXXX" value={form.phone} error={errors.phone} onChange={(value) => updateField("phone", value)} />
                                            </div>
                                        </section>
                                        ...
                                        <section>
                                            ...
                                            {savedAddresses.length > 0 && (
                                                <div className="space-y-2 mb-6">
                                                    {savedAddresses.map((savedAddress) => (
                                                        <label
                                                            key={savedAddress.id}
                                                            className={`flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all group ${selectedSavedAddressId === savedAddress.id ? "border-stone-800 bg-stone-50" : "border-stone-200 bg-white hover:border-stone-300"}`}
                                                        >
                                                            <div className={`mt-0.5 w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${selectedSavedAddressId === savedAddress.id ? "border-stone-800" : "border-stone-300"}`}>
                                                                {selectedSavedAddressId === savedAddress.id && <div className="w-2 h-2 rounded-full bg-stone-800" />}
                                                            </div>
                                                            <input type="radio" className="sr-only" checked={selectedSavedAddressId === savedAddress.id} onChange={() => applySavedAddress(savedAddress)} />
                                                            <div className="flex-1 min-w-0">
                                                                <p className="text-sm font-medium text-stone-800">{savedAddress.label}</p>
                                                                <p className="text-xs text-stone-400 mt-0.5">
                                                                    {[savedAddress.addressLine2, savedAddress.city, savedAddress.stateProvince, savedAddress.postalCode].filter(Boolean).join(", ")}
                                                                </p>
                                                            </div>
                                                            <button
                                                                type="button"
                                                                onClick={(e) => { e.preventDefault(); handleDeleteSavedAddress(savedAddress.id); }}
                                                    ...
                                                    <label className={`flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${selectedSavedAddressId === null ? "border-stone-800 bg-stone-50" : "border-stone-200 bg-white hover:border-stone-300"}`}>
                                                        <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${selectedSavedAddressId === null ? "border-stone-800" : "border-stone-300"}`}>
                                                            {selectedSavedAddressId === null && <div className="w-2 h-2 rounded-full bg-stone-800" />}
                                                        </div>
                                                        <input
                                                            type="radio"
                                                            className="sr-only"
                                                            ...
                                                            onChange={() => {
                                                                setSelectedSavedAddressId(null);
                                                                setForm((prev) => ({ ...prev, addressLine1: "", addressLine2: "", city: "", stateProvince: "", postalCode: "" }));
                                                            }}
                                                        />
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <FormField label="Address Line 1" name="addressLine1" placeholder="123 Main Street" value={form.addressLine1} error={errors.addressLine1} onChange={(value) => updateField("addressLine1", value)} />
                                                <FormField label="Address Line 2 (Identification)" name="addressLine2" placeholder="Apartment, floor, landmark…" value={form.addressLine2} error={errors.addressLine2} onChange={(value) => updateField("addressLine2", value)} />
                                                <FormField label="City" name="city" span="half" placeholder="Kathmandu" value={form.city} error={errors.city} onChange={(value) => updateField("city", value)} />
                                                <FormField label="State / Province" name="stateProvince" span="half" placeholder="Bagmati" value={form.stateProvince} error={errors.stateProvince} onChange={(value) => updateField("stateProvince", value)} />
                                                <FormField label="Postal Code" name="postalCode" span="half" placeholder="44600" value={form.postalCode} error={errors.postalCode} onChange={(value) => updateField("postalCode", value)} />

                                                <div className="col-span-2 flex items-center gap-2.5">
                                                    <Checkbox id="saveAddress" checked={saveForLater} onCheckedChange={(checked) => setSaveForLater(checked === true)} />
                                                    <Label htmlFor="saveAddress" className="text-sm text-stone-600 font-normal cursor-pointer">
                                                ...
                                                <div className="col-span-2">
                                                    ...
                                                    <Textarea rows={3} placeholder="Any special instructions..." value={form.notes} onChange={(e) => updateField("notes", e.target.value)} className="resize-none" />
                                                </div>
                                            </div>
                                        </section>

                                        <div>
                                            <Button onClick={() => { if (validate()) setStep("payment"); }} disabled={!canProceed} className="w-full">
                                                Continue to Payment
                                            </Button>
                                            {!canProceed && cart.length > 0 && (
                                                <p className="mt-2 text-center text-xs text-red-500">Remove unavailable items to proceed</p>
                                            )}
                                        </div>
                                    </div>
                                )}
                                ...
                                {step === "payment" && (
                                    <div className="space-y-8">
                                        <button onClick={() => setStep("details")} className="flex items-center gap-1.5 text-sm text-stone-400 hover:text-stone-700 transition-colors cursor-pointer">
                                        ...
                                        <div className="rounded-2xl bg-stone-50 border border-stone-100 p-4 text-sm">
                                            <p className="text-[11px] font-semibold uppercase tracking-widest text-stone-400 mb-3">Delivering to</p>
                                            <p className="text-stone-800 font-medium">{form.firstName} {form.lastName}</p>
                                            <p className="text-stone-500">{[form.addressLine1, form.addressLine2, form.city, form.stateProvince, form.postalCode].filter(Boolean).join(", ")}</p>
                                            <p className="text-stone-500">{form.email} · {form.phone}</p>
                                        </div>

                                        <Button onClick={() => handleSubmit(form)} disabled={!canProceed || isSubmitting} className="w-full">
                                            {isSubmitting ? (
                    ...
                    <div>
                        <div className="lg:sticky lg:top-24">
                            <p className="text-[11px] font-semibold uppercase tracking-widest text-stone-400 mb-4">Order Summary</p>
                                ...
                                <div className="px-5 divide-y divide-stone-50">
                                    {isPending ? (
                                        ...
                                        mergedItems.map((mergedItem) => {
                                            if (!mergedItem.product) return null;
                                            const validation = validations.find((validation) => validation.cartItemId === mergedItem.cartItemId);
                                            return (
                                                <OrderSummaryItem
                                                    key={mergedItem.cartItemId}
                                                    cartItem={mergedItem}
                                                    product={mergedItem.product}
                                ...
                                <div className="px-5 py-4 bg-stone-50 border-t border-stone-100 space-y-2">
                                    ...
                                    <div className="flex justify-between text-sm text-stone-500">
                                        ...
                                        <span className="tabular-nums">
                                            {shippingCost === 0 ? <span className="text-emerald-600 font-medium">Free</span> : `Rs. ${shippingCost.toFixed(2)}`}
                                        </span>
                                    </div>
                                    {shippingCost > 0 && <p className="text-[11px] text-stone-400">Free shipping on orders over Rs. 2,000</p>}
